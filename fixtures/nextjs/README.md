...
- Dev is v1 preview parity: the built worker served by workerd with `ASSETS`
  (run-worker-first), `WORKER_SELF_REFERENCE` (self service binding), and the
  same-script SQLite `DOQueueHandler` durable object. No HMR yet.
