 ...
 * (upstream/waku/packages/waku/src/adapters/cloudflare.ts @ f092454).
 *
 * The ONLY functional change: `buildEnhancers` no longer references
 * 'waku/adapters/cloudflare-build-enhancer', which is the sole code path in
 * waku that writes wrangler.jsonc / dist/server/wrangler.json /
 * .wrangler/deploy/config.json. Everything else (Hono app, cloudflare:workers
 * env access, SSG via the preview server) is byte-identical behavior.
 *
...
export default createServerEntryAdapter(
  (
    ...
    return {
      ...
      buildOptions,
      // Upstream: ['waku/adapters/cloudflare-build-enhancer'] — the ONLY
