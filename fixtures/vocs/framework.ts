 ...
 * `unstable_combinedPlugins`, so `@distilled.cloud/waku`'s Framework layer
 * cannot drive it directly. This layer mirrors that package's orchestration
 * (see packages/waku/src/Waku.ts) with vocs's plugin stack swapped in:
 *
 * - the deploy-target halves (wrangler-free adapter fork + cloudflare vite
 *   plugin pinned to waku's rsc entry) come from
 *   `@distilled.cloud/waku/cloudflare` — vocs uses waku's environments
 ...
 * - the waku-parity vite config that `@distilled.cloud/waku` carries through
...
import { WAKU_SERVER_ENTRY_MODULE } from "@distilled.cloud/waku";
import makeWakuCloudflareTarget from "@distilled.cloud/waku/cloudflare";
 ...
 * packages/waku/src/Waku.ts for the rationale: build and dev may run in the
...
/**
 * Waku-parity inline vite config. `@distilled.cloud/waku` injects these via
...
export const make = (
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const prepareTarget = Effect.fn(function* (root: string, phase: "build" | "dev") {
