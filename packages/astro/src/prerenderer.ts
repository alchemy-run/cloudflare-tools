...
export interface WorkerdPrerendererOptions {
  ...
  /**
   * The integration's `@distilled.cloud/cloudflare-vite-plugin` options: the
...
export function createWorkerdPrerenderer(options: WorkerdPrerendererOptions): AstroPrerenderer {
    ...
    if (serverUrl === undefined) {
      throw new Error(
        "The workerd prerender server is not running. " +
          "This is a bug in @distilled.cloud/astro — `setup()` must run before the prerender protocol is driven.",
      );
    }
    return serverUrl;
  };
  ...
  return {
    name: "@distilled.cloud/astro:workerd-prerenderer",
...
export const collectOutputModules = async (outputDir: string): Promise<Array<Module>> => {
  ...
  try {
    entries = await NodeFs.readdir(outputDir, { recursive: true, withFileTypes: true });
  } catch (cause) {
    throw new Error(
  ...
  if (entryIndex === -1) {
    throw new Error(
      `No prerender entry chunk (prerender-entry.*.mjs) found in "${outputDir}". ` +
        "This is a bug in @distilled.cloud/astro — the prerender environment should have built the worker server entrypoint.",
    );
  }
...
const moduleForFile = async (absolute: string, relative: string): Promise<Module | undefined> => {
  ...
  switch (extension) {
    case ".js":
    case ".mjs":
      return { name, type: "ESModule", content: await NodeFs.readFile(absolute, "utf8") };
    case ".cjs":
      return { name, type: "CommonJsModule", content: await NodeFs.readFile(absolute, "utf8") };
    case ".json":
      return { name, type: "Json", content: await NodeFs.readFile(absolute, "utf8") };
    case ".wasm":
      return { name, type: "Wasm", content: new Uint8Array(await NodeFs.readFile(absolute)) };
    default:
      return TEXT_EXTENSIONS.has(extension)
        ? { name, type: "Text", content: await NodeFs.readFile(absolute, "utf8") }
        : { name, type: "Data", content: new Uint8Array(await NodeFs.readFile(absolute)) };
  }
};
