 ...
 *   inside workerd (via `@distilled.cloud/cloudflare-vite-plugin`) and
...
const PROVIDER = "@distilled.cloud/astro/source";
    ...
    build: (ctx) =>
      Effect.gen(function* () {
        ...
        const bundle: BundleOutput | undefined =
          files.length > 0
            ? { files: files as [BundleFile, ...Array<BundleFile>], hash: bundleHash }
            : undefined;
