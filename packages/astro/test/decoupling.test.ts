 ...
 * Cloudflare imports — no `@distilled.cloud/cloudflare-*` packages, no
 ...
 * `@distilled.cloud/astro/cloudflare` subpath (and `./source`, which is the
...
describe("framework-generic core is cloudflare-free", () => {
  it("no module reachable from src/index.ts imports anything cloudflare", async () => {
    ...
    for (const [file, specifiers] of graph) {
      ...
      for (const specifier of specifiers) {
        // No cloudflare package, runtime module id, or types — not even
        // type-only imports.
        expect(specifier, `${relative} imports ${specifier}`).not.toMatch(/cloudflare/i);
      }
    }
  });
