...
interface CapturedSetup {
  session?: {
    driver?: { entrypoint?: string | { toString(): string }; config?: Record<string, unknown> };
  };
}
...
describe("zero-config sessions", () => {
  it("defaults the session driver to Cloudflare KV when unset", () => {
    ...
    expect(captured.session?.driver?.config).toMatchObject({ binding: "SESSION" });
  });

  it("uses the configured sessionKVBindingName", () => {
    const captured = runConfigSetup({ sessionKVBindingName: "MY_SESSION" }, "build");
    expect(captured.session?.driver?.config).toMatchObject({ binding: "MY_SESSION" });
  });

  it("leaves a user-configured driver untouched", () => {
    const userSession = { driver: "redis" };
    const captured = runConfigSetup(undefined, "build", { session: userSession });
  ...
  it("usesCloudflareKVSessionDriver matches by name and entrypoint", () => {
    ...
    expect(
      usesCloudflareKVSessionDriver({ driver: { entrypoint: "unstorage/drivers/redis" } }),
    ).toBe(false);
  });
...
describe("unsupported upstream options", () => {
  ...
  it("accepts the supported options", () => {
    expect(() =>
      distilledCloudflare({ sessions: false, sessionDevKV: false, sessionKVBindingName: "S" }),
    ).not.toThrow();
  });
});
...
describe("astro:config:done", () => {
  it("injects the cloudflare.d.ts types reference", async () => {
    ...
    expect(ctx.injected[0]?.content).toContain("@distilled.cloud/astro/types.d.ts");
  });
