...
import { makeIntegrationPluginOptions, SERVER_ENTRYPOINT } from "../src/cloudflare.ts";
...
describe("makeIntegrationPluginOptions (workerd prerendering)", () => {
  it("promotes prerender to a child worker environment during a workerd build", () => {
    ...
    expect(options.viteEnvironments).toEqual({ entry: "ssr", children: ["prerender"] });
...
describe("collectOutputModules", () => {
  ...
  const makeOutputDir = async (files: Record<string, string | Uint8Array>): Promise<string> => {
    ...
    for (const [relative, content] of Object.entries(files)) {
      ...
      await NodeFsPromises.mkdir(NodePath.dirname(absolute), { recursive: true });
  ...
  it("loads the output with the prerender entry chunk first", async () => {
    ...
    expect(byName.get("chunks/page_abc.mjs")).toMatchObject({ type: "ESModule" });
  ...
  it("fails with a clear error when the entry chunk is missing", async () => {
    const output = await makeOutputDir({ "chunks/page_abc.mjs": "export const page = 1;" });
