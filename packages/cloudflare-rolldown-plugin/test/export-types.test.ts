...
describe("export types virtual module", () => {
  ...
  it("resolves the importable specifier to the virtual id", () => {
    expect(resolveId(EXPORT_TYPES_MODULE_ID)).toEqual({ id: RESOLVED_EXPORT_TYPES_MODULE_ID });
...
describe("haveExportTypesChanged", () => {
  const previous: ExportTypes = { Counter: "DurableObject", Api: "WorkerEntrypoint" };

  it("is false for the same exports in a different order", () => {
    expect(
      haveExportTypesChanged(previous, { Api: "WorkerEntrypoint", Counter: "DurableObject" }),
    ).toBe(false);
  });

  it("is true when an export is added", () => {
    expect(haveExportTypesChanged(previous, { ...previous, Flow: "WorkflowEntrypoint" })).toBe(
      true,
    );
  });
  ...
  it("is true when an export is renamed", () => {
    expect(
      haveExportTypesChanged(previous, { Counter: "DurableObject", Rpc: "WorkerEntrypoint" }),
    ).toBe(true);
  });
});
