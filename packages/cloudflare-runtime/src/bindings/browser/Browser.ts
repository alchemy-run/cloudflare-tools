...
import * as BrowserWorker from "worker:./Browser.worker.ts";
  ...
  Effect.gen(function* () {
    ...
    const handler: LoopbackServer.RawHandler = async (req, res) => {
      ...
      switch (url.pathname) {
        ...
        default: {
          return respond(404, { error: `Unknown browser loopback route "${url.pathname}"` });
        }
      }
    };
              ...
              worker: {
                ...
                bindings: [
                  ...
                  },
                  {
                    ...
                    durableObjectNamespace: { className: BROWSER_SESSION_CLASS_NAME },
                  },
                ],
...
export async function launchBrowser({ headful }: { headful?: boolean }): Promise<{
  ...
  const { executablePath } = await installWithCorruptedCacheRecovery(installOptions);
