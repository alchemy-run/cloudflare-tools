 ...
 */
import type { BlobId } from "../../internal/shared.worker.ts";
import {
  assert,
  base64Decode,
  base64Encode,
...
interface KeyEntry<Metadata = unknown> {
  key: string;
  /** Milliseconds since unix epoch. */
  expiration?: number;
  metadata?: Metadata;
}
interface KeyValueEntry<Metadata = unknown> extends KeyEntry<Metadata> {
  value: ReadableStream<Uint8Array>;
}
interface KeyEntriesQuery {
  prefix?: string;
  cursor?: string;
  limit: number;
}
interface KeyEntries<Metadata = unknown> {
...
class KeyValueStorage<Metadata = unknown> {
  ...
  async get(key: string): Promise<KeyValueEntry<Metadata> | null> {
    ...
    const value = await this.#blob.get(row.blob_id);
    if (value === null) return null;
    return { ...entry, value };
  }

  async put(entry: KeyValueEntry<Metadata> & { signal?: AbortSignal }): Promise<void> {
...
export class KVNamespaceObject implements DurableObject {
  ...
  #list(url: URL): Response {
    ...
    if (res.cursor === undefined) {
      ...
      result = { keys, list_complete: false, cursor: res.cursor, cacheStatus: null };
    }
