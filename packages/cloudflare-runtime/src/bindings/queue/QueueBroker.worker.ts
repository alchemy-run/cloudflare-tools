...
interface BrokerEnv {
  [QUEUE_BROKER_BINDING]: DurableObjectNamespace;
  [QUEUE_USER_WORKER_BINDING]: Fetcher;
  [QUEUE_NAME_BINDING]: string;
  // workerd `json` bindings are injected pre-parsed, so these are objects.
  [QUEUE_CONSUMER_BINDING]?: QueueConsumer;
  [QUEUE_PRODUCERS_BINDING]?: ReadonlyArray<QueueProducerEntry>;
...
function deserialise(contentType: QueueContentType, bytes: Uint8Array): QueueBody {
  switch (contentType) {
    case "text":
...
function serialise(message: QueueMessage): QueueIncomingMessage {
...
function byteLength(body: QueueBody): number {
  switch (body.contentType) {
    case "text":
      return encoder.encode(body.body).byteLength;
    case "json":
      return encoder.encode(JSON.stringify(body.body)).byteLength;
    case "bytes":
 ...
 */
export class QueueBrokerObject extends DurableObject<BrokerEnv> {
  readonly #messages: Array<QueueMessage> = [];
  #pendingFlush?: PendingFlush;
  #backlogBytes = 0;

  get #name(): string {
    return this.env[QUEUE_NAME_BINDING];
  }

  get #consumerOptions(): QueueConsumer | undefined {
    return this.env[QUEUE_CONSUMER_BINDING];
  }

  get #producer(): QueueProducerEntry | undefined {
    const producers = this.env[QUEUE_PRODUCERS_BINDING] ?? [];
    return producers.find((producer) => producer.queueName === this.#name);
  }
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/batch") {
        return await this.#batch(request);
      }
  ...
  async #message(request: Request): Promise<Response> {
    const response = { metadata: { metrics: this.#metrics() } };
    ...
    const delay = this.#headerDelay(request) ?? this.#producer?.deliveryDelay;
    const bytes = new Uint8Array(await request.arrayBuffer());

    this.#enqueue([{ contentType, delaySecs: delay, body: bytesToBase64(bytes) }]);
    return Response.json(response);
  }

  async #batch(request: Request): Promise<Response> {
    const response = { metadata: { metrics: this.#metrics() } };
    if (this.#consumerOptions === undefined) {
      return Response.json(response);
    }
    ...
    const delay = this.#headerDelay(request) ?? this.#producer?.deliveryDelay;
    const body = (await request.json()) as QueuesBatchRequest;
    this.#enqueue(body.messages, delay);
    return Response.json(response);
  }

  #headerDelay(request: Request): number | undefined {
  ...
  #validateBatchSize(request: Request) {
  ...
  #enqueue(messages: ReadonlyArray<QueueIncomingMessage>, globalDelay = 0) {
    for (const message of messages) {
      const id = message.id ?? crypto.randomUUID().replace(/-/g, "");
      const timestamp = new Date(message.timestamp ?? Date.now());
      const body = deserialise(message.contentType, base64ToBytes(message.body));
  ...
  #flush = async (): Promise<void> => {
    ...
    try {
      response = await this.#dispatch(batch, metadata);
    } catch (error) {
      console.error(`Queue "${this.#name}" consumer threw:`, error);
      response = exceptionQueueResult;
    }
    ...
    for (const message of batch) {
      ...
      if (message.failedAttempts < maxAttempts) {
        ...
        console.warn(
          `Dropped message "${message.id}" on queue "${this.#name}" after ${maxAttempts} failed attempts`,
        );
      }
    }
  ...
  #dispatch(
    ...
    return this.env[QUEUE_USER_WORKER_BINDING].queue(this.#name, messages, metadata);
  }
  ...
  async #sendToDeadLetterQueue(
    ...
    const binding = this.env[queueDeadLetterBindingName(deadLetterQueue)] as Fetcher | undefined;
    if (binding === undefined) {
      console.warn(
        `Cannot move messages on queue "${this.#name}" to dead letter queue "${deadLetterQueue}": no binding configured`,
      );
      return;
    }
    const request: QueuesBatchRequest = { messages: messages.map(serialise) };
    ...
    if (!response.ok) {
      console.warn(
        `Failed to move messages on queue "${this.#name}" to dead letter queue "${deadLetterQueue}": HTTP ${response.status}`,
      );
    }
  }
}
...
export default {
  async fetch(request, env) {
    const stub = env[QUEUE_BROKER_BINDING].getByName(QUEUE_BROKER_SINGLETON);
    const body =
