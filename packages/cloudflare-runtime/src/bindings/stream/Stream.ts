...
import * as StreamWorker from "worker:./Stream.worker.ts";
  ...
  Effect.gen(function* () {
    ...
    const handler: LoopbackServer.RawHandler = (req, res) => {
      ...
      res
        ...
        .end(JSON.stringify({ error: `Unknown stream loopback route "${url.pathname}"` }));
    };
            ...
            register: () =>
              Plugin.use(Loopback.Loopback, (loopback) =>
                Effect.map(loopback.api.route(LOOPBACK_TARGET_STREAM, handler), (service) => {
                  ...
                  return { name: SERVICE_STREAM, entrypoint: STREAM_BINDING_ENTRYPOINT };
                }),
              ),
          },
          start: (ports) =>
                ...
                bindings: [
                  {
                    ...
                    durableObjectNamespace: { className: STREAM_OBJECT_CLASS_NAME },
                  },
                  {
                  ...
                  ...(enableControlEndpoints
                    ? [{ name: BINDING_STREAM_ENABLE_CONTROL_ENDPOINTS, json: "true" }]
                    : []),
                ],
              },
            };
              ...
              middlewares: [
                {
                  ...
                  worker: {
                    ...
                    bindings: [
                      {
                        name: "STREAM",
                        service: { name: SERVICE_STREAM, entrypoint: STREAM_BINDING_ENTRYPOINT },
                      },
                    ],
                  },
