...
import { WORKERS_MODULE_SYMBOL } from "./symbols.shared.ts";
import type { __RPC_TARGET_BRAND } from "./types.shared.ts";
...
export class ErrorStubHook extends StubHook {
  ...
  map(path: PropertyPath, captures: Array<StubHook>, instructions: Array<unknown>): StubHook {
    return this;
  }
...
const PROXY_HANDLERS: ProxyHandler<{ raw: RpcStub }> = {
  apply(target: { raw: RpcStub }, thisArg: any, argumentsList: Array<any>) {
    let stub = target.raw;
...
export class RpcPayload {
  ...
  public static fromArray(array: Array<RpcPayload>): RpcPayload {
    let hooks: Array<StubHook> = [];
    let promises: Array<LocatedPromise> = [];
  ...
  // without copying.
  public static forEvaluate(hooks: Array<StubHook>, promises: Array<LocatedPromise>) {
  ...
  public getHookForWritableStream(
  ...
  public getHookForReadableStream(
  ...
  private static deliverRpcPromiseTo(
    ...
    if (inner instanceof RpcPayload) {
      ...
      promises.push(
        inner.then((payload) => {
          let subPromises: Array<Promise<unknown>> = [];
...
export class PayloadStubHook extends ValueStubHook {
  ...
  dup(): StubHook {
    ...
    return new PayloadStubHook(RpcPayload.deepCopyFrom(thisPayload.value, undefined, thisPayload));
  }
