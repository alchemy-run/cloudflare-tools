...
import type { LocatedPromise, RpcTarget, StubHook } from "./core.shared.ts";
import {
  PayloadStubHook,
  PromiseStubHook,
  RpcPayload,
...
export class Devaluator {
  ...
  public static devaluate(
    value: unknown,
    parent?: object,
    exporter: Exporter = NULL_EXPORTER,
    source?: RpcPayload,
  ): unknown {
        ...
        switch (value[0]) {
          ...
          case "headers":
            ...
            if (value.length === 2 && value[1] instanceof Array) {
              return new Headers(value[1] as Array<[string, string]>);
            }
            break;
          ...
          case "remap": {
            ...
            let captures: Array<StubHook> = value[3].map((cap) => {
              if (
