...
export type EncodedValue =
  ...
  | { readonly $: "number"; readonly value: number | "NaN" | "Infinity" | "-Infinity" }
...
export type EnvBindingDescriptor =
  | { readonly name: string; readonly kind: "value"; readonly value: EncodedValue }
  | { readonly name: string; readonly kind: "stub"; readonly className?: string };
...
export const encodeValue = (
  ...
  if (value instanceof ArrayBuffer) {
    return { $: "bytes", kind: "arraybuffer", base64: bytesToBase64(new Uint8Array(value)) };
  }
  ...
  if (Array.isArray(value)) {
    return { $: "array", value: value.map((entry) => encodeValue(entry, encodeUnknown)) };
  }
