    ...
    Effect.acquireRelease(
      Effect.sync(() => {
        ...
        watcher.on("event", (event) => {
          if (event.code === "ERROR") {
            Queue.offerUnsafe(queue, Result.fail(bundleErrorFromUnknown(event.error)));
  ...
  return (content: (importPath: string) => string) => {
    ...
    return {
      ...
      options(inputOptions) {
        assert(typeof inputOptions.input === "string", "input must be a string");
...
export function bundleOutputFromRolldownOutputBundle(
  ...
  return Effect.forEach(
    files as [rolldown.OutputChunk, ...Array<rolldown.OutputChunk | rolldown.OutputAsset>],
...
function bundleOutputFromFiles(
  files: [BundleFile, ...Array<BundleFile>],
...
function sha256(data: string | Uint8Array<ArrayBufferLike>): Effect.Effect<string> {
  return Effect.sync(() => crypto.createHash("sha256").update(data).digest("hex"));
}
