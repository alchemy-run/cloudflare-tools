...
const producerWorker = (name: string, consumer: string, marker: string) => ({
  ...
  name,
  tails: [consumer],
  modules: [{ name: "main.js", type: "ESModule", content: producerScript(marker) } as const],
});
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* waitForRegistryEntry({ kind: "worker", scriptName: "tails-consumer" });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* waitForRegistryEntry({ kind: "worker", scriptName: "tails-late-consumer" });
