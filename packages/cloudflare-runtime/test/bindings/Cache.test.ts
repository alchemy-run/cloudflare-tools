...
import * as Cache from "../../src/bindings/cache/index.ts";
...
class TestCache {
  ...
  async match(key: string | CacheKey): Promise<MatchedResponse | undefined> {
    const result = (await this.#call({ method: "match", key: this.#key(key) })) as {
      status: number;
  ...
  async delete(key: string | CacheKey): Promise<boolean> {
    return (await this.#call({ method: "delete", key: this.#key(key) })) as boolean;
  }
}
...
layer(CacheTestLayer)("Cache binding", (it) => {
  ...
  cacheTest("match returns cached responses", async (ctx) => {
    ...
    await cache.put(
      key,
      textResponse("body", { "Cache-Control": "max-age=3600", "X-Key": "value" }),
    );
    ...
    await cache.put(key, {
      body: { kind: "stream", base64: Buffer.from("streamed").toString("base64") },
      headers: { "Cache-Control": "max-age=3600" },
    });
    res = await cache.match(key);
    assert(res !== undefined);
  ...
  cacheTest("match returns empty response", async (ctx) => {
    ...
    await cache.put(key, { body: null, headers: { "Cache-Control": "max-age=3600" } });
  ...
  cacheTest("match respects If-None-Match header", async (ctx) => {
    ...
    await cache.put(
      key,
      textResponse("body", { ETag: '"thing"', "Cache-Control": "max-age=3600" }),
    );
