...
import * as D1 from "../../src/bindings/d1/index.ts";
...
class TestD1Database {
  ...
  batch<T = unknown>(statements: Array<TestD1PreparedStatement>): Promise<Array<D1Result<T>>> {
    return this.send({ method: "batch", statements: statements.map((s) => s.toJSON()) });
  }
...
class TestD1PreparedStatement {
  ...
  first<T = unknown>(column?: string): Promise<T | null> {
    return this.db.send({ method: "prepare", action: "first", column, ...this.toJSON() });
  }
  ...
  raw<T = unknown>(options?: { columnNames?: boolean }): Promise<Array<T>> {
    return this.db.send({ method: "prepare", action: "raw", options, ...this.toJSON() });
  }
}
