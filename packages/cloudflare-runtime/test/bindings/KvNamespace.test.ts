...
async function encodeValue(value: PutValue): Promise<EncodedValue> {
  ...
  if (value instanceof ArrayBuffer) {
    return { kind: "arrayBuffer", base64: Buffer.from(value).toString("base64") };
  }
...
class NamespacedKv {
  ...
  async getWithMetadata(key: string | Array<string>, options?: unknown): Promise<any> {
    return this.#stripNs(
      await this.#call({ method: "getWithMetadata", key: this.#keys(key), options }),
    );
  }
...
const controlBinding = Effect.succeed({
  ...
  service: {
    ...
    props: {
      json: JSON.stringify({ namespaceId: "namespace" } satisfies KvNamespace.KvServiceProps),
    },
  },
});
    ...
    await testList(ctx, {
      ...
      pages: [
        [
          ...
          { name: "key2", expiration: TIME_FUTURE + 100, metadata: { testing: 2 } },
          { name: "key3", expiration: TIME_FUTURE + 200, metadata: { testing: 3 } },
        ],
      ],
    });
  });
