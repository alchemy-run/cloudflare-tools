...
class NamespacedR2 {
  ...
  async delete(keys: string | Array<string>): Promise<void> {
    if (typeof keys === "string") {
      ...
      await this.call({ method: "delete", keys: keys.map((key) => this.ns + key) });
    }
  }
  ...
  async list(options?: R2ListOptions): Promise<ListResult> {
    ...
    return { ...result, objects: result.objects.map((object) => decodeObject(object)) };
  }
...
const controlBinding = Effect.succeed({
  ...
  service: {
    ...
    props: {
      json: JSON.stringify({ bucketName: "bucket" } satisfies R2Bucket.R2ServiceProps),
    },
  },
});
