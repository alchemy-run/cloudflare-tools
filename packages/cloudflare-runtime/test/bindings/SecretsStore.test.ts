...
import * as SecretsStore from "../../src/bindings/secrets-store/index.ts";
    ...
    Effect.gen(function* () {
      const worker = yield* startTestWorker({
        ...
        bindings: [
          ...
          SecretsStore.admin({ binding: "ADMIN", storeId: "shared_identifier" }),
          ...
          function* (run: (worker: TestWorker) => Effect.Effect<void>) {
            const worker = yield* startTestWorker({
              ...
              bindings: [
                ...
                SecretsStore.admin({ binding: "ADMIN", storeId: "persist_store" }),
              ],
            });
            yield* run(worker);
          },
