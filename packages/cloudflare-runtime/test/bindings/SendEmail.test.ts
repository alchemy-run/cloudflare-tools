...
import * as SendEmail from "../../src/bindings/send-email/index.ts";
...
layer(localRuntimeLayer)("SendEmail binding", (it) => {
  it.effect("unbound send_email accepts a valid message and synthesizes a messageId", () =>
    Effect.gen(function* () {
      ...
      const body = (yield* Effect.promise(() => res.json())) as { messageId: string };
  ...
  it.effect("single allowed destination: mismatched destination is rejected", () =>
    Effect.gen(function* () {
      const worker = yield* startTestWorker({
        ...
        compatibilityFlags: [],
        bindings: [
          SendEmail.local({ binding: "SEND_EMAIL", destinationAddress: "helly.r@example.com" }),
        ],
        modules: [...modules],
      });
      const res = yield* worker.fetch(
    ...
    Effect.gen(function* () {
      ...
      const send = (to: unknown) =>
        worker.fetch("/builder", {
          method: "POST",
          body: JSON.stringify({ from: "sender@example.com", to, subject: "Test", text: "Test" }),
        });
      ...
      const namedRejected = yield* send({ name: "Blocked User", email: "blocked@example.com" });
  ...
  it.effect("MessageBuilder respects allowed sender addresses (plain and named)", () =>
    Effect.gen(function* () {
      ...
      const send = (from: unknown) =>
        worker.fetch("/builder", {
          method: "POST",
          body: JSON.stringify({ from, to: "recipient@example.com", subject: "Test", text: "T" }),
        });

      expect((yield* send("allowed@example.com")).status).toBe(200);
      expect((yield* send({ name: "Allowed Sender", email: "allowed@example.com" })).status).toBe(
        200,
      );
      ...
      const namedRejected = yield* send({ name: "Blocked", email: "blocked@example.com" });
      expect(namedRejected.status).toBe(500);
      expect(yield* Effect.promise(() => namedRejected.text())).toContain("not allowed");
    }),
  );
