...
const sendCmd = <T>(worker: TestWorker, op: string, args: Record<string, unknown> = {}) =>
  Effect.gen(function* () {
    const body = yield* worker.fetchJson<
      | { ok: true; result: T }
      | { ok: false; error: string; name?: string; code?: number; statusCode?: number }
...
const uploadVideo = (worker: TestWorker, videoUrl: URL, params?: Record<string, unknown>) =>
  sendCmd<Video>(worker, "upload", { url: videoUrl.toString(), ...(params ? { params } : {}) });
  ...
  describe("Stream videos", () => {
    it.effect("upload and retrieve details", () =>
      Effect.gen(function* () {
        ...
        const details = yield* sendCmd<Video>(worker, "video.details", { id: video.id });
    ...
    it.effect("delete video", () =>
      Effect.gen(function* () {
        ...
        const message = yield* sendCmdError(worker, "video.details", { id: video.id });
        expect(message).toContain("Video not found");
      }).pipe(Effect.scoped),
    );
    ...
    it.effect("generate token", () =>
      Effect.gen(function* () {
        ...
        const video = yield* uploadVideo(worker, videoUrl);
        const token = yield* sendCmd<string>(worker, "video.generateToken", { id: video.id });
  ...
  describe("Stream captions", () => {
    ...
    it.effect("list captions", () =>
      Effect.gen(function* () {
        ...
        const video = yield* uploadVideo(worker, videoUrl);
        yield* sendCmd(worker, "captions.generate", { id: video.id, language: "en" });
        yield* sendCmd(worker, "captions.generate", { id: video.id, language: "fr" });
    ...
    it.effect("delete caption", () =>
      Effect.gen(function* () {
        ...
        const video = yield* uploadVideo(worker, videoUrl);
        yield* sendCmd(worker, "captions.generate", { id: video.id, language: "en" });
        yield* sendCmd(worker, "captions.delete", { id: video.id, language: "en" });
    ...
    it.effect("upload caption is idempotent (upsert)", () =>
      Effect.gen(function* () {
        ...
        yield* sendCmd(worker, "captions.upload", { id: video.id, language: "en" });
        yield* sendCmd(worker, "captions.upload", { id: video.id, language: "en" });
  ...
  describe("Stream watermarks", () => {
    ...
    it.effect("get watermark", () =>
      Effect.gen(function* () {
        ...
        const fetched = yield* sendCmd<Watermark>(worker, "watermarks.get", { id: created.id });
  ...
  describe("Stream downloads", () => {
    ...
    it.effect("get downloads", () =>
      Effect.gen(function* () {
        ...
        const video = yield* uploadVideo(worker, videoUrl);
        yield* sendCmd(worker, "downloads.generate", { id: video.id, type: "default" });
        yield* sendCmd(worker, "downloads.generate", { id: video.id, type: "audio" });
    ...
    it.effect("delete download", () =>
      Effect.gen(function* () {
        ...
        const video = yield* uploadVideo(worker, videoUrl);
        yield* sendCmd(worker, "downloads.generate", { id: video.id, type: "default" });
        yield* sendCmd(worker, "downloads.delete", { id: video.id, type: "default" });
  ...
  describe("Stream deletes clean up properly", () => {
    ...
    it.effect("deleting video cleans up its blob from storage", () =>
      Effect.gen(function* () {
        ...
        const details = yield* sendCmd<Video>(worker, "video.details", { id: video2.id });
    ...
    it.effect("deleting one video does not affect another video's captions and downloads", () =>
      Effect.gen(function* () {
        ...
        yield* sendCmd(worker, "captions.generate", { id: videoA.id, language: "en" });
        yield* sendCmd(worker, "captions.generate", { id: videoB.id, language: "en" });
        yield* sendCmd(worker, "downloads.generate", { id: videoA.id, type: "default" });
        yield* sendCmd(worker, "downloads.generate", { id: videoB.id, type: "default" });
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runAgainstStorage((worker) =>
          Effect.gen(function* () {
            assert(videoId);
            const details = yield* sendCmd<Video>(worker, "video.details", { id: videoId });
