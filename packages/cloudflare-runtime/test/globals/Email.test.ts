...
layer(localRuntimeLayer)("inbound email trigger route", (it) => {
  it.effect("dispatches a valid email to the email() handler", () =>
    Effect.gen(function* () {
      ...
      const res = yield* worker.fetch(EMAIL_PATH, { method: "POST", body: VALID_EMAIL });
  ...
  it.effect("rejects an unparseable email", () =>
    Effect.gen(function* () {
      ...
      const res = yield* worker.fetch(EMAIL_PATH, { method: "POST", body: "adfsedfhwiofe" });
      expect(res.status).toBe(400);
      expect(yield* Effect.promise(() => res.text())).toBe(
  ...
  it.effect("rejects an email without a Message-ID header", () =>
    Effect.gen(function* () {
      ...
      const res = yield* worker.fetch(EMAIL_PATH, { method: "POST", body: email });
      expect(res.status).toBe(400);
      expect(yield* Effect.promise(() => res.text())).toBe(
        "Email could not be parsed: invalid or no message id provided",
      );
  ...
  it.effect("404s unknown /cdn-cgi/handler/ paths with a hint", () =>
    Effect.gen(function* () {
      ...
      const res = yield* worker.fetch("/cdn-cgi/handler/bogus", { method: "POST" });
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          const res = yield* worker.fetch(EMAIL_PATH, { method: "POST", body: incomingEmail });
          expect(yield* Effect.promise(() => res.text())).toBe(
