...
layer(localRuntimeLayer, { excludeTestServices: true })("PlatformProxy", (it) => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(env.CONFIG).toEqual({ nested: { flag: true }, values: [1, 2, 3] });
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          expect(await DO.get(DO.idFromName("proxy-test")).increment(2)).toBe(2);
          expect(await DO.get(DO.idFromName("proxy-test")).increment()).toBe(3);
          ...
          await expect(DO.get(DO.idFromName("proxy-test"))).rejects.toThrow(/cannot serialize/);
        });
      }),
    { timeout: 30_000 },
  );
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          await expect(
            proxy.caches.default.put(key, new Response("partial", { status: 206 })),
          ).rejects.toThrow(/206/);
        });
      }),
    { timeout: 30_000 },
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(() => detached(Promise.resolve())).toThrow("Illegal invocation");
      }),
    { timeout: 30_000 },
  );
});
