...
layer(services, { excludeTestServices: true })((it) => {
  ...
  it.effect("forwards x-forwarded-host and x-forwarded-proto headers to the upstream worker", () =>
    Effect.gen(function* () {
      const proxy = yield* WorkerProxy.WorkerProxy;
      const upstream = yield* serveUpstream(HTTP_WORKER);
      const instance = yield* proxy.serve();
  ...
  it.effect("returns immediately when the upstream fails with a non-retryable error", () =>
    Effect.gen(function* () {
      const proxy = yield* WorkerProxy.WorkerProxy;
      const instance = yield* proxy.serve();
  ...
  it.effect("rejects controller requests with a missing or invalid authorization token", () =>
    Effect.gen(function* () {
      const proxy = yield* WorkerProxy.WorkerProxy;
      const instance = yield* proxy.serve();
