...
- **Framework packages** (`@distilled.cloud/waku`, `.../astro`,
...
- **Per-framework target modules** own the halves that are neither generic
  ...
  subpath of its framework package (e.g. `@distilled.cloud/waku/cloudflare`
  exporting the `WakuCloudflareTarget`). A future AWS target is a new subpath
  (`@distilled.cloud/waku/aws`) implementing the same seams — no change to
...
```ts
// packages/waku/src/cloudflare.ts (illustrative)
...
1. Its options gain `target?: <Framework>TargetInput` (a
   ...
   cloudflare subpath specifier (e.g. `"@distilled.cloud/waku/cloudflare"`).
...
4. Never import a platform package (`@distilled.cloud/cloudflare-vite-plugin`,
...
(`packages/tools/e2e/src/CloudflareTarget.ts`) — serving built output is the
...
`./cloudflare` subpath), create `src/cloudflare.ts` + a `./cloudflare` export
...
- **Waku** — `WakuTarget` extends `DeployTarget` with the adapter-path hook
  (today: the wrangler-free fork at `@distilled.cloud/waku/adapter`, selected
