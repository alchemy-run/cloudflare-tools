...
/**
 * The synthetic module prefix `@distilled.cloud/cloudflare-rolldown-plugin`
  ...
  Effect.gen(function* () {
    ...
    const readFile = (path: string, name: string): Effect.Effect<OutputFile, CollectorError> =>
      fs.readFileString(path).pipe(
        ...
        Effect.mapError(
          (error) => new CollectorError({ message: "Failed to read file", cause: error }),
        ),
      );
    ...
    const collectInMemory = (): Effect.Effect<Array<OutputFile> | undefined, CollectorError> => {
      ...
      return Effect.all(Array.from(serverModules.values()), { concurrency: "unbounded" }).pipe(
        Effect.map((modules) => sortServerModules(modules, serverEntry)),
      );
    };
...
export const readServerModulesFromDisk = (
  ...
  Effect.gen(function* () {
    ...
    const readError = (error: unknown) =>
      new CollectorError({ message: "Failed to read server modules from disk", cause: error });
...
export const collectExternalWorkspaces = (
  ...
  Effect.gen(function* () {
    ...
    return yield* Effect.forEach(directories, (directory) =>
      ...
      Effect.mapError(
        (error) =>
          new CollectorError({ message: "Failed to collect external workspaces", cause: error }),
      ),
    );
  });
