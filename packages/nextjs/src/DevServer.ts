 ...
 */
import type { BindingHooks, WorkerdLogging } from "@distilled.cloud/cloudflare-runtime";
import * as PlatformProxy from "@distilled.cloud/cloudflare-runtime/platform-proxy/PlatformProxy";
import type * as Runtime from "@distilled.cloud/cloudflare-runtime/Runtime";
...
export const start = Effect.fn("Nextjs.DevServer.start")(function* (options: DevServerOptions) {
  ...
  yield* plantCloudflareContext({ env: proxy.env, cf: proxy.cf, ctx: proxy.ctx });
  ...
  return { url: new URL(`http://${urlHost}:${http.port}`) } satisfies DevServerInstance;
}) as (
