 ...
 * incremental cache).
 */
import type { BindingHooks, RuntimeWorker } from "@distilled.cloud/cloudflare-runtime";
...
const PROVIDER = "@distilled.cloud/nextjs/source";
...
/**
 * The JSON-serializable options of the `@distilled.cloud/nextjs/source`
 ...
 */
const hashInputTree = Effect.fn(function* (root: string, options: NextjsSourceOptions) {
  const fs = yield* FileSystem.FileSystem;
...
const frameworkError = (cause: FrameworkCore.FrameworkError): SourceProviderError =>
  new SourceProviderError({ provider: PROVIDER, message: cause.message, cause });
...
const makeProvider = (options: NextjsSourceOptions): SourceProvider => {
  ...
  return {
    ...
    dev: Effect.fn(function* (ctx) {
      ...
      return { mode: "server", url: new URL(server.url) } satisfies SourceDevHandle;
    }),
  };
};
