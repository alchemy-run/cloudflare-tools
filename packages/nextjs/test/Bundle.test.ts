...
import { bundleWorker, CREATE_REQUIRE_BANNER, WORKER_ENTRY_NAME } from "../src/Bundle.ts";
...
const makeOpenNextFixture = (root: string) => {
  ...
  NodeFs.mkdirSync(NodePath.join(openNext, ".build", "durable-objects"), { recursive: true });
  NodeFs.mkdirSync(NodePath.join(openNext, "server-functions", "default"), { recursive: true });
