...
Workers is the built-in target (`@distilled.cloud/nuxt/cloudflare`); the
...
`@distilled.cloud/nuxt/cloudflare`) implements the contract:
...
```ts
// 1. default — omitted: resolves "@distilled.cloud/nuxt/cloudflare"
//    from the project's node_modules
Nuxt.layer({});

// 2. factory — applied to the config the framework assembles from options
import cloudflareTarget from "@distilled.cloud/nuxt/cloudflare";
