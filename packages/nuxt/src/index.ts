/**
 * `@distilled.cloud/nuxt` — Nuxt integration implementing framework-core's
 * `Framework` service, with the deploy target passed as a value (Cloudflare
 * Workers by default, via the `@distilled.cloud/nuxt/cloudflare` subpath
 * module).
 *
 ...
 * behind `@distilled.cloud/nuxt/cloudflare`.
 */
...
/**
 * The e2e-harness factory contract (`framework: "@distilled.cloud/nuxt"` in
 * `e2e.config.ts`).
 */
const factory = (
