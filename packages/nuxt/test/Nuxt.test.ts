  ...
  it("maps a nitro .output tree onto the BuildOutput contract, entry first", async () => {
    const output = await runWithNode(
      Effect.gen(function* () {
        ...
        const dir = yield* fs.makeTempDirectoryScoped({ prefix: "nuxt-output-" });
        const serverDir = path.join(dir, "server");
        const publicDir = path.join(dir, "public");
        yield* fs.makeDirectory(path.join(serverDir, "chunks"), { recursive: true });
    ...
    const result = await runWithNode(
      Effect.gen(function* () {
        ...
        return yield* Effect.result(
          readNitroOutput({ dir, serverDir, publicDir: path.join(dir, "public") }),
        );
      }),
    );
...
describe("fromHarnessOptions", () => {
  ...
  it("falls back to the vite alias when no target carriage exists", () => {
    const options = fromHarnessOptions({ vite: { compatibilityDate: "2026-03-10" } });
    expect(options.compatibilityDate).toBe("2026-03-10");
