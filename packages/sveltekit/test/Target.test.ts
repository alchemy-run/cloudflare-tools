...
import { isDeployTarget, resolveDeployTarget } from "@distilled.cloud/framework-core";
import type { BuildOutput } from "@distilled.cloud/framework-core";
...
describe("makeCloudflareTarget", () => {
  ...
  it("produces the in-memory kit adapter from the adapter hook", () => {
    const cloudflare = makeCloudflareTarget({ adapter: { assetsBinding: "STATIC" } });
    const adapter = cloudflare.adapter({ root: "/tmp/project" });
    expect(adapter.name).toBe("@distilled.cloud/sveltekit");
...
describe("resolveDeployTarget interop", () => {
  ...
  it("accepts a prebuilt target value as-is", async () => {
    const value = makeCloudflareTarget({ compatibilityFlags: ["nodejs_compat"] });
