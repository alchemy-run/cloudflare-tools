import { Framework, FrameworkError, loadProjectModule } from "@distilled.cloud/framework-core";
...
export const resolve = Effect.fn(function* (options: Options.Options) {
  ...
  if (typeof input === "string") {
    ...
    const module_ = yield* loadProjectModule<FrameworkModule>(cwd, input).pipe(
      Effect.mapError(
        (error) =>
          new FrameworkError({ framework: input, message: error.message, cause: error.cause }),
      ),
    );
