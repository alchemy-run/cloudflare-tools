...
`@distilled.cloud/framework-core`'s `Framework` service — programmatic
...
`@distilled.cloud/waku/cloudflare`.

## Architecture: framework half × deploy-target half

The package separates two concerns (see
`packages/framework-core/README.md` → "Architecture: frameworks × deploy
targets" for the full doctrine):

| Module                               | Role                                                                                                                                           | Platform imports                   |
| ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| `.` (`src/index.ts`, `src/Waku.ts`)  | **Framework half**: waku config synthesis, programmatic build/dev orchestration, `BuildOutput` collection, deploy-target resolution            | **None** (enforced by a unit test) |
| `./cloudflare` (`src/cloudflare.ts`) | **Cloudflare target**: `WakuTarget` implementation — injects `@distilled.cloud/cloudflare-vite-plugin`, selects the `./adapter` fork           | `cloudflare-vite-plugin`           |
| `./adapter` (`src/adapter.ts`)       | Wrangler-free fork of `waku/adapters/cloudflare`; bundled _into the worker_ by waku's vite pipeline                                            | worker-side only                   |
| `./source` (`src/source.ts`)         | Alchemy `Cloudflare.Worker` source provider (`{ provider: "@distilled.cloud/waku/source" }`) wrapping the framework with the cloudflare target | `cloudflare-runtime` (types)       |
...
### The cloudflare target (`@distilled.cloud/waku/cloudflare`)
...
- produces `@distilled.cloud/cloudflare-vite-plugin` with `main` pinned to
...
```ts
import wakuFramework, { make } from "@distilled.cloud/waku";

make({
  /** Deploy target: value | (config) => target | module specifier |
   *  harness carriage. Default: "@distilled.cloud/waku/cloudflare",
...
- a **target value** — full type safety over the config; build it yourself:
  ...
  ```ts
  import cloudflareTarget from "@distilled.cloud/waku/cloudflare";
