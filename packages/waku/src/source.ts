 ...
 * (`{ provider: "@distilled.cloud/waku/source", options }` on alchemy's
...
const readClientAssets = Effect.fn(function* (
  ...
  return {
    ...
    _headers,
    _redirects,
    hash: sha256Object({ config, manifest: sortedManifest, _headers, _redirects }),
...
const PROVIDER = "@distilled.cloud/waku/source";
...
export const makeWakuSourceProvider = (options: WakuSourceOptions): SourceProvider => {
    ...
    dev: Effect.fn(function* (ctx) {
      ...
      const server = yield* withRootCwd(
        ...
        Effect.gen(function* () {
          ...
          Effect.mapError(asProviderError("Failed to start the waku dev server")),
        ),
      );
      return { mode: "server", url: new URL(server.url) } satisfies ServerDevHandle;
    }),
  };
};
